...
/// Explorer for the Threads connected to this Fleet.
///
...
/// across the Thread. Turning selected documents into input/output pairs comes
/// with the gRPC ingestion work — the transport and the browsing are in place.
struct ThreadSourcePanel: View {
    ...
    @State private var groups: [ThreadGroupSummary] = []
    @State private var groupsHasMore = false
    @State private var loadingMore = false
    @State private var groupDocuments: [String: [ThreadDocument]] = [:]
    @State private var searchQuery = ""
    @State private var searchResults: [ThreadPartition] = []
    @State private var selected: [String: ThreadDocument] = [:]
    ...
    var body: some View {
        ...
        .onChange(of: appState.connectedThreads) { _, threads in
            // Auto-pick when exactly one Thread is connected; drop a stale selection.
            if selectedThreadId == nil, threads.count == 1 { selectedThreadId = threads.first?.id }
            if let id = selectedThreadId, !threads.contains(where: { $0.id == id }) { selectedThreadId = nil }
        }
    }
        ...
        FleetCard(padding: 14) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Spacer()
                    if appState.threadServerRunning {
                        Button("Stop") { Task { await appState.stopThreadServer() } }
                            .buttonStyle(.fleetQuiet)
                    } else {
                        Button("Start") { Task { await appState.startThreadServer() } }
                            .buttonStyle(.fleet)
                    }
                }
                ...
                HStack(spacing: 8) {
                    Stepper("port \(appState.threadServerPort)",
                            value: $appState.threadServerPort, in: 1024 ... 65535)
                        .font(.fleetMono(10))
                    if appState.threadServerRunning {
                        Button("Restart") { Task { await appState.restartThreadServer() } }
                            .buttonStyle(.fleetQuiet)
                    }
                }

                // Row 3 — connected Thread picker (when any have dialed in).
                if appState.threadServerRunning, !appState.connectedThreads.isEmpty {
                    Picker("Thread", selection: $selectedThreadId) {
                        Text("Select a Thread…").tag(UUID?.none)
                        ForEach(appState.connectedThreads) { thread in
                            Text("\(thread.id.uuidString.prefix(8)) · \(thread.host):\(thread.grpcPort)")
                                .tag(UUID?.some(thread.id))
                        }
                    }
                    .labelsHidden()
                }
    ...
    private var statusColor: Color {
        if appState.threadServerError != nil { return Color.fleetError }
        if !appState.threadServerRunning { return Color.fleetInk.opacity(0.25) }
        return appState.connectedThreads.isEmpty ? Color.fleetGold : Color.fleetGreen
    }

    private var statusText: String {
        if let error = appState.threadServerError { return error }
        if !appState.threadServerRunning { return "Stopped" }
        return "Listening · :\(appState.threadServerPort)"
    }
    ...
    private var connectHint: some View {
        VStack(spacing: 12) {
            Spacer()
            FleetMark(size: 30)
            Text(appState.threadServerRunning ? "Waiting for a Thread" : "Server stopped")
                .font(.fleetSerif(16, weight: .light, italic: true))
                .foregroundStyle(Color.fleetInk)
            Text("Point a Thread at this host:port:\n--fleet-host 127.0.0.1 --fleet-grpc-port \(appState.threadServerPort)")
                .font(.fleetMono(9.5))
    ...
    @ViewBuilder
    private func groupBody(_ group: ThreadGroupSummary) -> some View {
    ...
    private var search: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                TextField("Search the Thread…", text: $searchQuery)
    ...
    private func documentRow(_ document: ThreadDocument) -> some View {
    ...
    private func partitionRow(_ partition: ThreadPartition) -> some View {
        HStack(alignment: .top, spacing: 8) {
            VStack(alignment: .leading, spacing: 2) {
                Text(partition.text)
    ...
    private var footer: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(
                "Fleet trains on paired input/output JSON. Turning Thread documents into pairs "
    ...
    private func toggle(_ document: ThreadDocument) {
    ...
    private func loadLibrary() async {
        ...
        do {
            ...
            let page = try await appState.threadImporter().library(threadId: threadId, ownerId: ownerId)
            groups = page.groups
    ...
    private func loadMoreGroups() async {
        guard let threadId = selectedThreadId, groupsHasMore, !loadingMore else { return }
        loadingMore = true; status = "Loading more…"
        do {
            // Cursor = the last group's id (the server returns id-sorted groups after it).
            let page = try await appState.threadImporter()
                .library(threadId: threadId, ownerId: ownerId, afterId: groups.last?.id ?? "")
    ...
    private func loadDocuments(_ group: ThreadGroupSummary) async {
        guard let threadId = selectedThreadId else { return }
        busy = true; status = "Loading documents…"
        do {
            let fetch = try await appState.threadImporter()
                .documents(threadId: threadId, ownerId: ownerId, documentIds: group.documents.map(\.id))
            ...
            // The Thread omits documents this owner may not read rather than failing.
    ...
    private func runSearch() async {
        ...
        do {
            searchResults = try await appState.threadImporter()
                .search(threadId: threadId, query: query, ownerId: ownerId)
