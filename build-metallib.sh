#!/bin/bash
# Compiles Frigate's vendored MLX Metal shaders into mlx.metallib and places it
# next to the built binary. MLX loads this at runtime for GPU execution; without
# it you'll see "Failed to load the default metallib".
#
# Run this once after `swift build` (or `swift build -c release`):
#   ./build-metallib.sh [debug|release]   (default: debug)
#
# Override the Frigate location with FRIGATE_DIR=/path/to/Frigate if it is not the
# sibling directory ../Frigate.
